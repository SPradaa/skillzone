...
$zone = $conn->prepare("SELECT * FROM zonas");
$zone->execute();
$zonas = $zone->fetchAll(PDO::FETCH_ASSOC);


$conZona = $conn->prepare("SELECT * FROM zonas LIMIT 6");
$conZona->execute();
$zonas = $conZona->fetchAll(PDO::FETCH_ASSOC);
...
<body>
    ...
    <main class="main">
            ...
            <div class="selectores">
                ...
                <form action="" method="POST" class="form">
                    <div class="contend-inputs">
                        <label for="servicio" class="tikets"> Que Necesitas ? <br> <br>
                        ...
                        <label for="zonas" class="tikets"> ¿En que Zona ? <br><br>
                            <select name="zonas" id="zonas" class="options">
                                <option value="">Seleccione una zona</option>
                                <?php
                                foreach ($zonas as $zona) {
                                    echo '<option value="' . $zona['id_zona'] . '">' . $zona['zona'] . '-' . $zona['ciudad'] . '</option>';
                                }
                                ?>
                            </select>
                        </label>
        ...
        <section class="provedoresDestacados">
            ...
            <?php
            $conUser = $conn->prepare("SELECT * FROM provedores LIMIT 6");
            $conUser->execute();
            $provedores = $conUser->fetchAll(PDO::FETCH_ASSOC);
            ?>

            <?php
            foreach ($provedores as $provedor) {
                  $clasificacion = $conn->prepare("SELECT * FROM categorias WHERE id_categoria = :id_categoria");
            $clasificacion->execute(['id_categoria' => $provedor['id_categoria']]); 
            $class = $clasificacion->fetch(PDO::FETCH_ASSOC);
          
                $calificacion = strlen($provedor['calificacion']);
                echo '<div class="provedores"><img src="' . $provedor['foto'] . '" alt="fotografia de '.$provedor['nombre']. '
                " class="img-provedor"> <p class="nombre-provedor" >' . $provedor['nombre'] . '</p>
                <p class="calificacion-provedor"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path fill="#eab308" d="m12 17.275l-4.15 2.5q-.275.175-.575.15t-.525-.2t-.35-.437t-.05-.588l1.1-4.725L3.775 10.8q-.25-.225-.312-.513t.037-.562t.3-.45t.55-.225l4.85-.425l1.875-4.45q.125-.3.388-.45t.537-.15t.537.15t.388.45l1.875 4.45l4.85.425q.35.05.55.225t.3.45t.038.563t-.313.512l-3.675 3.175l1.1 4.725q.075.325-.05.588t-.35.437t-.525.2t-.575-.15z"/></svg>
                ' . $provedor['calificacion']. ' / ' . $calificacion . '</p>
                <p class="categoria-provedor">' . $class['categoria'] . '</p>
                <p class="precio-provedor">'. $provedor['precio'] . ' / Hora</p>
                </div>';
            }
            ?>  
            </div>
